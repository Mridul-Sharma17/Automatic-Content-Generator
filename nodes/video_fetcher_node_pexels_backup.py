...
class VideoFetcherNode:
    ...
    def _select_best_video(self, videos: List[Dict]) -> Optional[Dict]:
        ...
        logger.info(f"📊 Video scoring results for query:")
        for i, video_data in enumerate(suitable_videos[:5]):  # Show top 5
            video = video_data['video']
            logger.info(f"  {i+1}. ID:{video['id']} Score:{video_data['score']:.1f} "
                       f"{video['width']}x{video['height']} {video['duration']}s")
    ...
    def _calculate_video_score(self, video: Dict, video_file: Dict) -> float:
        """Calculate a score for video quality and suitability with detailed logging"""
        score = 0.0
        score_breakdown = []
        ...
        resolution_points = min(resolution_score, 1.0) * 30
        score += resolution_points
        score_breakdown.append(f"Resolution({width}x{height}): +{resolution_points:.1f}")
        ...
        if height > 0:
            ...
            aspect_points = aspect_score * 25
            score += aspect_points
            score_breakdown.append(f"Aspect({aspect_ratio:.2f}): +{aspect_points:.1f}")
        ...
        score_breakdown.append(f"Duration({duration}s): +{duration_points}")
        ...
        score_breakdown.append(f"Quality({quality}): +{quality_points}")
        ...
        if 'mp4' in file_type:
            format_points = 10
        score += format_points
        score_breakdown.append(f"Format({file_type}): +{format_points}")
        
        logger.debug(f"Video {video.get('id')} score breakdown: {' | '.join(score_breakdown)} = {score:.2f}")
        
        # CRITICAL DEBUG: Print score calculation to console for debugging
        print(f"🔍 DEBUG Score for video {video.get('id')}: {' | '.join(score_breakdown)} = {score:.2f}")
